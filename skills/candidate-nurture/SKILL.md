...
description: >
  候选人保温 + 面评催收。读每日对账预警数据（停滞/面评欠收/待推进）+ 信号文件（LLM判读的邀约/拒绝+用户决策）+ 保温状态文件（谁碰过几次/该不该升级），
  ...
  LLM消息判读（recruit-followup做，本skill只读其落盘的_signals.json）、
...
但缺少的是：**有了预警数据（谁停滞了、谁欠面评了）之后，配套的"怎么碰"**。`_daily_review.py` 已经算出了谁该跟进，本 skill 补上"今天该碰谁 + 用什么话术碰"。
...
**前置条件**：
1. 先跑 `python notes/_daily_review.py`（不写表）生成 `_daily_review.json`。报告不存在或非当天时提示用户先跑对账。
2. 如果 recruit-followup 早晨对账已执行过（含LLM判读），`_signals.json` 应存在。不存在时降级运行（只用预警数据+保温状态，不交叉比对信号），但提示用户"信号文件不存在，未闭环邀约可能遗漏"。
...
**降级**：`_signals.json` 不存在时，只用预警数据+保温状态运行，提示用户"信号文件不存在，未闭环邀约可能遗漏"。
...
python "C:/Users/wuchunbo/.agents/skills/candidate-nurture/scripts/nurture_state.py" --stale
```
...
```bash
# 记录触达（每个执行的候选人都记）
python "C:/Users/wuchunbo/.agents/skills/candidate-nurture/scripts/nurture_state.py" \
