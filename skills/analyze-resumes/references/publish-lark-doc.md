...
| 2 | **删除「## 需人工看原文件」里的本地路径**，只保留姓名 + issue 描述 | 飞书用户打不开 `<PROJECT_ROOT>/...`；需要看原件的，让 HR 找 Agent 要 |
...
- `YOUR_OPEN_ID` 是吴春波 open_id（AGENTS.md 关键路径表已固化）
...
| 失败点 | 兜底动作 |
|--------|---------|
| import 失败（格式/大小/权限） | 用 `im +messages-send` 发文字："飞书发布失败（原因：{err}），本地报告：<PROJECT_ROOT>/notes/简历评估_{M.DD}.md" |
...
- **不要直接 import 原始 md** — 必须先预处理剔除本地路径和归档说明，否则飞书文档里会出现 `<PROJECT_ROOT>/...` 这种用户打不开的噪音
