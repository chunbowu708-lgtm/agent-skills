...
> 生产者：`F:/miniwanob/notes/_daily_review.py`（recruit-followup skill 调度）
> 产物：`F:/miniwanob/notes/_daily_review.json`
...
| 列表 | 含义 | 每项字段 |
|---|---|---|
| `ats` | ATS 全量在途投递（**人员主数据，看板漏斗/矩阵数据源**） | `talent_id, name, job, dept, stage, dwell_days, interview_count, latest_conclusion` |
| ~~`ats[].stage_type`~~ | **不输出**（dashboard 需要时按 `stage` 字符串归一化，见 `_normalize_stage`） | — |
| `stuck` | 卡住 ≥2 天 | `talent_id, name, stage, dwell_days, level, reason`。**reason 含"⚠️零记录"= 暗坑**：面试阶段零面试记录（罗海贵类），既不在 feedback_overdue 也不在 to_advance，最易漏 |
...
| `today_interviews` | 今日面试 | `talent_id, name, time, summary, in_track` |
...
`source, sender, sender_is_wubo, time, content_full, mentions_wubo`

- `content_full` 是完整原文（不截断——截断吃过亏，见对抗审查报告）。
- 意图判读（邀约/拒绝/待定）是 **L3 语义判断**：交主会话 LLM 做，禁止关键词匹配（2026-06-30 对抗审查：词表方案 9 误报 5 漏报、拒绝信号归零）。
...
| 不存在时 | candidate-nurture 降级运行（只用预警+保温状态），提示"未闭环邀约可能遗漏" |
...
> 生产者：`F:/miniwanob/notes/_hire.py`（recruit-followup 录入流程）
> 产物：`F:/miniwanob/notes/_hire_result.json`
