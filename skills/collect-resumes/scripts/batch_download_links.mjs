...
//   node batch_download_links.mjs --manifest F:/miniwanob/notes/collection_manifest.json
//
//   # 指定 record ID（逗号分隔）
...
import { DOWNLOADS_DIR, MANIFEST as MANIFEST_DEFAULT } from './lib/paths.mjs';
...
async function batchDownload(tasks) {
  ...
  // playwright 装在项目根 F:/miniwanob，skill 目录找不到，用绝对路径加载
  let chromium;
  try {
    ({ chromium } = await import('playwright'));
  } catch {
    const pwPath = 'F:/miniwanob/node_modules/playwright/index.mjs';
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    try {
      ...
      results.push({ ...task, status: 'ok', file: savePath });
