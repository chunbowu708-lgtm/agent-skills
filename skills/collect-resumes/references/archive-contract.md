...
```
$ARCHIVE_ROOT/                              ← F:/miniwanob/data/在招岗位候选人管理
├── {项目团队}/                             ← 如"山海弹珠项目"
...
| 项 | 约定 | 例 |
|---|---|---|
| 日期目录 | `{M.DD}_{N}份`，N=本目录人头数（按姓名去重） | `7.01_7份` |
| 已收集简历夹 | 统一名 `已收集简历`（历史遗留的"收集到简历"不改） | — |
| 档位子目录 | 评估后按档位 `mv` 进这三个标准名之一：`强推` / `可推` / `待定·不推`（待定和不推共置） | `强推/` |
| 简历文件名 | 正职 `{姓名}_{岗位简称}_{经验}年.pdf`；实习 `{姓名}_{岗位简称}_{届}.pdf`。**不含薪资** | `林艺_游戏发行运营实习生_27届.pdf` |
...
1. **数量**：`_N份` 标注 == 实际人头数（档位目录+同级文件同时统计；无标注 → STOP）
2. **姓名**：文件名姓名 ∈ 简历正文署名（挡 MID 填串）
3. **薪酬**：所有可承载内容的格式（PDF/DOCX/ZIP 包内）都扫描，不得残留薪酬段
4. **格式**：PDF/DOCX/ZIP 均通过统一提取器；图片型 PDF OCR 不可用 → STOP；不支持/损坏/加密 → STOP（统称"阻断文件"）
...
> 产物：`F:/miniwanob/notes/collection_manifest.json`
...
| `target_filename` | 附件/链接 | 归档目标文件名（resolve_records 填） |
| `url` | 链接 | 提取到的下载链接 |
| `candidate_name` | 全部 | resolve_records 绑定的候选人名 |
| `job` | 全部 | resolve_records 绑定的岗位 |
| `target_path` | 全部 | 归档目标完整路径 |
| `sha256` | 全部 | 下载后校验的内容哈希（download 阶段填） |
| `errors` | 全部 | `[{code, message, at}]` 错误历史（blocked 时追加） |
| `exclude_reason` | 全部 | `{code, message}` 排除原因（excluded 时必填） |

**状态机**（`lib/manifest.mjs:16-26`，禁止跳过）：
```
...
**消费约定**：所有阶段脚本通过 `record_id` 定位记录，状态只能按状态机推进（`transitionRecord` 校验，非法转换抛 `INVALID_TRANSITION`）。`verify_archive` 的 manifest 闭环校验要求所有 record 达到 `validated` 终态。
